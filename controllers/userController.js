...
exports.getStats = async (req, res, next) => {
  try {
    ...
    const topJobs = await Application.aggregate([
      ...
      {
        $lookup: {
          from: 'jobposts',        // MongoDB collection name (lowercase + plural)
          localField: '_id',       // field in applications to match on
          foreignField: '_id',     // field in jobposts to match against
          as: 'jobInfo',           // name of the resulting joined array
        },
      },
      { $unwind: '$jobInfo' },
      {
